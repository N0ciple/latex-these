...
\DIFdelbegin %DIFDELCMD < \acp{DNN} %%%
\DIFdelend \DIFaddbegin \acfp{DNN} \DIFaddend which have their roots in \DIFdelbegin %DIFDELCMD < \acp{ANN}%%%
...
connections between neurons in the \ac{MLP} form a \DIFdelbegin %DIFDELCMD < \acf{DAG}%%%
\DIFdelend \DIFaddbegin \DIFadd{directed acyclic graph}\DIFaddend , where
...
For a network with L layers and parameters \DIFdelbegin \DIFdel{$\theta={\mathbf{w}_1, \dots,
\mathbf{w}_L}$}\DIFdelend \DIFaddbegin \DIFadd{$\theta=\{\mathbf{w}_1, \dots,
\mathbf{w}_L\}$}\DIFaddend , the $\ell_1$ and $\ell_2$ regularisation term is defined as
follows:\\
...
$\gamma$ and smoothes the variations of the descent direction\DIFdelbegin \DIFdel{and thus, }\DIFdelend \DIFaddbegin \DIFadd{, thus }\DIFaddend preventing
the optimisation \DIFdelbegin \DIFdel{to get }\DIFdelend \DIFaddbegin \DIFadd{from getting }\DIFaddend stuck in small local \DIFdelbegin \DIFdel{minimums}\DIFdelend \DIFaddbegin \DIFadd{minima}\DIFaddend . The momentum term is
