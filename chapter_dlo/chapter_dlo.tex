...
connected \DIFdelbegin \DIFdel{upstream to }\DIFdelend \DIFaddbegin \DIFadd{downstream from }\DIFaddend an input layer and \DIFdelbegin \DIFdel{downstream }\DIFdelend \DIFaddbegin \DIFadd{upstream }\DIFaddend to an output layer. Each
...
\DIFdelbegin \DIFdel{maximises }\DIFdelend \DIFaddbegin \DIFadd{maximise }\DIFaddend a performance, quantified by a metric $P$, often based on the
...
perfectly predict the output of the training set but \DIFaddbegin \DIFadd{may }\DIFaddend fail to generalise to
...
\begin{equation}
  ...
  \mathcal{R}_{\ell_2}(\theta) = \frac{\lambda}{2} \displaystyle \sum_{i=1}^{L} \| \mathbf{w}_i \|_2\DIFaddbegin \DIFadd{^2 
}\DIFaddend \end{equation}\\

\DIFaddbegin \noindent \DIFadd{where $\left|. \right|$ and $\|.\|_2^2$ respectively denote the sum of
the absolute value and the sum of the squaring of each element of the vector.}\\

\DIFaddend \subsection{Loss Optimisation}\label{sec:dlo:backpropagation}
...
\begin{algorithm}
  ...
  \begin{algorithmic}
    ...
    $\theta^{(0)}$, \DIFdelbegin \DIFdel{$n \geq m$ }\DIFdelend \DIFaddbegin \DIFadd{$m' \geq m$ }\DIFaddend training pairs $(X,y)\in
...
A \ac{FC} layer can be \DIFdelbegin \DIFdel{describe }\DIFdelend \DIFaddbegin \DIFadd{described }\DIFaddend as a matrix-vector product as in
...
\noindent \textbf{Activation \DIFdelbegin \DIFdel{function}\DIFdelend \DIFaddbegin \DIFadd{functions}\DIFaddend .} \DIFdelbegin \DIFdel{These functions }\DIFdelend \DIFaddbegin \DIFadd{They }\DIFaddend are often applied to the output
feature map of a convolutional or fully connected layer, resulting in the
\emph{activation map} or \emph{activations}. \DIFdelbegin \DIFdel{This function introduces
}\DIFdelend \DIFaddbegin \DIFadd{These functions introduce
}\DIFaddend non-linearity into the model, allowing it to learn more complex patterns
...
\noindent \textbf{Pooling.} \DIFdelbegin \DIFdel{Pooling }\DIFdelend \DIFaddbegin \DIFadd{This operation }\DIFaddend is often employed after \ac{CL}
...
issue in LeNet-5 \DIFdelbegin \DIFdel{with the use of }\DIFdelend \DIFaddbegin \DIFadd{using }\DIFaddend data augmentation and dropout techniques\DIFdelbegin \DIFdel{and
introduced it also
introduced and popularised }\DIFdelend \DIFaddbegin \DIFadd{, while also
introducing and popularising }\DIFaddend the \ac{ReLU} activation function.\\
...
led to the \emph{vanishing \DIFdelbegin \DIFdel{gradients}\DIFdelend \DIFaddbegin \DIFadd{gradient}\DIFaddend } problems\DIFdelbegin \DIFdel{. The vanishing gradients problem
}\DIFdelend \DIFaddbegin \DIFadd{, which }\DIFaddend refers to the situation in
...
the vanishing \DIFdelbegin \DIFdel{gradients  }\DIFdelend \DIFaddbegin \DIFadd{gradient }\DIFaddend issue with its novel inception modules, which allowed
...
\begin{figure}[htbp]
  ...
  \caption{A residual block and its skip connection used in
    ...
    mitigating the \DIFdelbeginFL \emph{\DIFdelFL{vanishing gradients}} %DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \emph{\DIFaddFL{vanishing gradient}} \DIFaddendFL problem.}
...
(detailed subsequently)\DIFdelbegin \DIFdel{2 each}\DIFdelend , while ResNet20, represented in \cref{fig:dlo:resnet20},
...
\begin{figure}[htbp]
  ...
  \caption{A sample \DIFdelbeginFL \DIFdelFL{grid }\DIFdelendFL of images from \DIFdelbeginFL \DIFdelFL{the }\DIFdelendFL CIFAR-100\DIFdelbeginFL \DIFdelFL{dataset}\DIFdelendFL . \DIFdelbeginFL \DIFdelFL{The images
    represent a subset }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Each image represents an instance
    of one }\DIFaddendFL of the 100 \DIFdelbeginFL \DIFdelFL{available }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{distinct }\DIFaddendFL classes\DIFdelbeginFL \DIFdelFL{, each image represents
    a unique class}\DIFdelendFL .}
...
TinyImageNet \DIFdelbegin \DIFdel{dataset }\DIFdelend is another popular dataset in machine learning and computer vision,
...
validation and testing. The training set serves to \DIFdelbegin \DIFdel{train }\DIFdelend \DIFaddbegin \DIFadd{tune the weights of }\DIFaddend the
