...
\begin{abstract}
  ...
  techniques, pruning is a popular approach that seeks to reduce the size of
  neural networks by removing redundant or unnecessary weights. However, most of
  the pruning methods rely on saliency indicators that identify removable
  weights after training without considering the targeted pruning rate.\\
  ...
  reparametrization acts as a mask that soft-prunes the smallest weights, while
  the budget loss serves as a surrogate $L_0$ norm that regulates the network
  budget. As a result, our approach achieves a much smaller performance drop
  with respect to the unpruned network compared to other pruning methods. We
  demonstrate experimentally the effectiveness of our method across various
  pruning rates, datasets, and architectures, including Conv4, VGG16, ResNet20
  on CIFAR10 and CIFAR100, and ResNet18 on the more challenging TinyImageNet
  dataset.\\
  \indent We also evaluate the effectiveness and relevance of our method in a
  comparative experimental analysis using different settings. Furthermore,
  we evaluate the proposed approach in a fine-tuning scenario and show that it
  outperforms common fine-tuning methods. Our results show that our
  proposed approach is a promising alternative to existing pruning techniques,
  providing an efficient and effective way to reduce the size of neural
  networks while eliminating the need for costly fine-tuning steps.\\
...
more complex and included multiple layers, known as multi-layer perceptron or
% TODO: retirer "Fully Connected networks" ? != MLP
% TODO utiliser un acronyme pour MLP ?
fully connected network. Then, with the emergence of Convolutional Neural
Networks (CNNs), the complexity of neural network architectures for image
recognition significantly increased. \acp{CNN} use convolutional layers to
...
network's size, memory and power consumption.\\
...
In order to address the aforementioned issues, we introduce a novel weight
...
\ldots, \mathbf{w}_L\}$, where $L$ is its number of layers. Each layer weight
tensor $\mathbf{w}_i$ contains $\nu_i$ elemens. $X$ is
the input given to the network.  $(X,y)$ is an element of a dataset
...
When used as a loss function, the $L_0$ norm is perfectly suited for introducing
...
\begin{equation}
  ...
  C(\{\mathbf{w}_1,\dots, \mathbf{w}_L\}) = \displaystyle \sum_{i=1}^{L} \sum_{j=1}^{\nu_i} h_t(w_{ij}). 
\end{equation} \\
...
$$\displaystyle\sum_{i=1}^{L}\frac{\displaystyle \sum_{j=1}^{\nu_i}
h_t(w_{ij})}{\nu_i}$$ 
However, the number of elements in a layer greatly varies
from one layer to another (as demonstrated in
...
To establish the importance and the effectiveness of the budget loss in our
...
where the budget loss was removed, and \emph{(ii)} a variant where the budget
loss is replaced with a regularisation loss based on the $L_1$ norm of the
network weights. To remove the budget loss, the value of $\lambda$ is set to
...
loss conditioning as in \cref{sec:chap1:budget_loss}, the $L_1$ norm is divided
...
\begin{equation}
  ...
  \mathcal{L} = \mathcal{L}_{task} + \lambda \cdot \frac{1}{N} \sum_{i=1}^{L} || \mathbf{w}_i ||_1
\end{equation}

Where $||~.~||_1$ represents the $L_1$ norm, and $N$ is the total number of
...
- \emph{$L_1$ reg.}) also impacts negatively the performance, with the exception
...
the $L_1$ regulatisation does not target a precise budget, however, it still
...
the $L_1$ regularisation loss is to be controlled only by the parameter
...
\begin{figure}
  ...
  \subfloat[Conv4 CIFAR10\label{fig:chap1:no_budget_conv4_cifar10}]{
    ...
    The experimental results are denoted \emph{$L_1$ reg.}, wherein the budget
    loss is replaced by a $L_1$ regularisation loss on the network weights. The
...
experiments by varying the mixing coefficient $\lambda$ from 0.5 to 50. The
...
\cref{fig:chap1:budget_only_conv4,fig:chap1:budget_only_resnet20,fig:chap1:budget_only_vgg16}, we only display the results for $\lambda = 5$.\\
...
method and standard fine-tuning \cite{DBLP:conf/nips/HanPTD15}. This setup is
...
results of our results indicates increased robustness against the inherent
randomness of the optimization process. Consequently, the final performance is
less dependent on specific initializations, batch orders, or data augmentation
seeds than with standard fine-tuning approaches.\\


This reduces the susceptibility of the
method to the random initialisation of the network and the random choice of the
training set. \\ 
...
budget loss to enhance the network's robustness to pruning:  our
reparametrisation and budget loss significantly reduce the network performance
drop induced by pruning compared to a network trained without. This forces less
useful weights to take small values, binding the weight value to the network
