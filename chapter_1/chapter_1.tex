\chapter{Weight Reparametrisation \DIFaddbegin \DIFadd{and Budget Loss for Network Pruning without Fine-Tuning}\DIFaddend }
\label{chap:chapter1}
...
\section{Introduction \DIFaddbegin \DIFadd{and Related Work}\DIFaddend }
...
loss that drives the sparsity toward the predefined pruning rate.\DIFdelbegin %DIFDELCMD < 
...
%DIFDELCMD < %%%
%DIF <  region
%DIF <  Neural networks have disrupted the field of machine learning, leading to
%DIF <  breakthroughs in various areas such as image and speech recognition
%DIF <  \cite{DBLP:conf/nips/KrizhevskySH12,DBLP:journals/corr/SimonyanZ14a,DBLP:conf/cvpr/HeZRS16,DBLP:journals/corr/HannunCCCDEPSSCN14,DBLP:conf/icassp/ChanJLV16,DBLP:conf/icml/AmodeiABCCCCCCD16},
%DIF <  natural language processing
%DIF <  \cite{DBLP:conf/emnlp/BudzianowskiV19,DBLP:conf/naacl/DevlinCLT19,DBLP:conf/nips/VaswaniSPUJGKP17},
%DIF <  as well as exotic domains like game playing
%DIF <  \cite{silver2016mastering,silver2018general} or molecule folding
%DIF <  \cite{jumper2021highly}. However, the size of neural networks has steadily
%DIF <  increased in recent years, thanks to the availability of powerful \acp{GPU}. The
%DIF <  latter makes possible the training of larger and more complex models. As the
%DIF <  complexity and scale of neural networks have expanded, there has been a
%DIF <  corresponding increase in the computational and memory demands necessary for
%DIF <  their effective training and deployment. Origins of neural network architectures
%DIF <  are rooted in the Rosenblatt Perceptron \cite{rosenblatt1958perceptron}, which
%DIF <  is a single-layer feedforward network consisting of only one neuron. Over time,
%DIF <  neural network architectures became more complex and included multiple layers,
%DIF <  known as \ac{MLP} or fully connected network. Then, with the emergence of
%DIF <  \acp{CNN}, the complexity of neural network architectures for image recognition
%DIF <  significantly increased. \acp{CNN} use convolutional layers to automatically and
%DIF <  adaptively learn spatial hierarchies of features from input images. This
%DIF <  adaptivity allows \acp{CNN} to effectively learn and classify images with high
%DIF <  accuracy. This enhancement in accuracy, however, is accompanied by a rise in
%DIF <  complexity and, notably, a higher number of parameters. Noteworthy \ac{CNN}
%DIF <  architectures typically encompass millions of parameters. For instance, AlexNet
%DIF <  (2012) comprises 60 million parameters \cite{DBLP:conf/nips/KrizhevskySH12}, VGG
%DIF <  networks (2014) is composed of 132 to 143 million parameters
%DIF <  \cite{DBLP:journals/corr/SimonyanZ14a}, Inception (2014) incorporates 27 million
%DIF <  parameters in its third version \cite{DBLP:conf/cvpr/SzegedyLJSRAEVR15} and
%DIF <  ResNet networks (2015) span from 11 to 60 million parameters
%DIF <  \cite{DBLP:conf/cvpr/HeZRS16}.\\
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  The demand for neural networks in embedded applications has been rapidly
%DIF <  increasing in recent years, driven in part by the widespread use of mobile
%DIF <  devices. As these devices have become more advanced and ubiquitous, there has
%DIF <  been a growing need for intelligent applications capable of executing intricate
%DIF <  tasks in various contexts on the said devices. Such complex tasks include object
%DIF <  detection in self-driving cars
%DIF <  \cite{howard2017mobilenets,DBLP:conf/icml/TanL19,DBLP:conf/cvpr/RedmonDGF16,DBLP:conf/nips/RenHGS15,kuutti2020survey},
%DIF <  image and speech recognition in mobile devices
%DIF <  \cite{kim2020review,mcgraw2016personalized}, and text-to-speech models in smart
%DIF <  speakers \cite{arik2017deep,ren2019fastspeech}. These applications require
%DIF <  real-time processing and low power consumption, which are impossible with large
%DIF <  neural networks. Pruning techniques can reduce the size of neural networks,
%DIF <  making them more suitable for deployment on embedded devices while still
%DIF <  maintaining or improving their performance. Methods such as structured
%DIF <  \cite{DBLP:conf/iclr/0022KDSG17,DBLP:conf/iccv/LiuLSHYZ17} and unstructured
%DIF <  weight pruning
%DIF <  \cite{DBLP:conf/nips/CunDS89,DBLP:conf/iclr/FrankleC19,DBLP:conf/nips/HanPTD15}
%DIF <  can reduce the number of parameters and \acp{FLOP}, consequently reducing the
%DIF <  network size, memory and power consumption.\\
...
network \cite{DBLP:conf/cvpr/HeZRS16}, are removed. \DIFaddbegin \DIFadd{Our method belongs to the
...
pruning without fine-tuning, followed by the contributions of this chapter.}\DIFaddend \\


\DIFdelbegin %DIFDELCMD < \noindent%%%
\textbf{\DIFdel{Unstructured magnitude pruning.}} %DIFAUXCMD
\DIFdel{It }\DIFdelend \DIFaddbegin \subsection{\DIFadd{Unstructured Magnitude Pruning.}} 
\DIFadd{Unstructured magnitude pruning }\DIFaddend has emerged as an effective heuristic for
...
\DIFdelbegin %DIFDELCMD < \noindent %%%
\textbf{\DIFdel{Weight reparametrisation.}} %DIFAUXCMD
\DIFdel{it }\DIFdelend \DIFaddbegin \subsection{\DIFadd{Weight Reparametrisation}} 
\DIFadd{Weight reparametrisation }\DIFaddend is a technique where the weights are expressed as a
...
reparametrisation based on raising a weight to the power of $\alpha$ while
...
\DIFdelbegin %DIFDELCMD < \noindent %%%
\textbf{\DIFdel{Pruning with budget.}} %DIFAUXCMD
\DIFdelend \DIFaddbegin \subsection{\DIFadd{Pruning with Budget}} 
\DIFaddend Most pruning techniques, including the ones presented in this section, enforce a
...
\DIFdelbegin %DIFDELCMD < \noindent%%%
\textbf{\DIFdel{Pruning without fine-tuning.}} %DIFAUXCMD
\DIFdelend \DIFaddbegin \subsection{\DIFadd{Pruning without fine-tuning}} 
\DIFaddend Most pruning methods cause a performance drop following the pruning step. This
...
Indeed, separating the optimisation of \DIFdelbegin \DIFdel{the }\DIFdelend topology from weight tuning is
...
are given in \cref{sec:chap1:overview}. \DIFdelbegin %DIFDELCMD < \cref{sec:chap1:experiments} %%%
\DIFdelend \DIFaddbegin \Cref{sec:chap1:experiments} \DIFaddend presents
the results of our comprehensive experiments, including performance \DIFdelbegin \DIFdel{benchmarks}\DIFdelend \DIFaddbegin \DIFadd{comparisons}\DIFaddend ,
...
budget is not possible since \DIFaddbegin \DIFadd{the }\DIFaddend $\ell_0$ norm is not differentiable. Thus, we
...
reparametrisation also helps \DIFdelbegin \DIFdel{minimising }\DIFdelend \DIFaddbegin \DIFadd{to minimise }\DIFaddend the performance drop between the
original and the pruned surrogate networks by maintaining competitive
performances without fine-tuning. Learning the \DIFdelbegin \DIFdel{weights }\DIFdelend \DIFaddbegin \DIFadd{weight }\DIFaddend values and the network
...
with an already trained and pruned \DIFdelbegin \DIFdel{initalisation}\DIFdelend \DIFaddbegin \DIFadd{initialisation}\DIFaddend . In this precise setup, since
...
Note that in the \emph{budget only} variant\DIFaddbegin \DIFadd{, }\DIFaddend the budget evaluation uses our
reparametrisation function as a surrogate $\ell_0$ norm but the weights used to
\DIFdelbegin \DIFdel{poduce }\DIFdelend \DIFaddbegin \DIFadd{produce }\DIFaddend the network output are the standard weights $\mathbf{w}$, not the
...
The results presented in this comparison and the \DIFdelbegin \DIFdel{one }\DIFdelend \DIFaddbegin \DIFadd{ones }\DIFaddend of
...
resulting in further a drop \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{in }\DIFaddend network capacity with weights of vanishingly
...
that the \DIFdelbegin \DIFdel{colored }\DIFdelend \DIFaddbegin \DIFadd{coloured }\DIFaddend area around the solid line is barely visible on the graphs.
This is not the case for \DIFdelbegin \DIFdel{the }\DIFdelend conventional fine-tuning where performances vary
...
\begin{figure}
  ...
  \caption{ Comparison of fine-tuning a network whose initialisation has been
    ...
    fine-tuning an unpruned \DIFdelbeginFL \DIFdelFL{initalisation }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{initialisation }\DIFaddendFL in the tested configurations.}
