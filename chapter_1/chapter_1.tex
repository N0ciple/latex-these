...
\begin{abstract}
  This chapter addresses the challenge of reducing the size of neural networks,
  which have grown in size with the increase of computational power. Although
  large neural networks have shown impressive performance across various
  domains, they are not deployable on embedded devices due to their size. Among
  the existing techniques, pruning is a popular approach to reduce the size of
  neural networks by removing redundant or unnecessary weights. However, most
  pruning methods rely on a saliency indicator that identifies the weights to be
  removed after the training process, without consideration of the target
  pruning rate.\\
  \indent In this chapter, we propose an alternative approach that embeds the
  pruning criterion within a weight reparametrization technique. In addition,
  our method incorporates a budget loss to drive sparsity towards the target
  pruning rate during training. The weight reparametrization acts as a mask
  that soft-prunes the smallest weights, while the budget loss serves as a
  surrogate $L_0$ norm to regulate the network's budget. As a result, our
  approach achieves a much smaller performance drop compared to other pruning
  methods. We demonstrate experimentally the effectiveness of our method across
  various pruning rates, datasets, and architectures, including Conv4, VGG16,
  ResNet20 on CIFAR10 and CIFAR100, and ResNet18 on the more complex
  TinyImageNet dataset.\\
  \indent We also evaluate the effectiveness and relevance of our method's two
  components in a comparative experimental analysis with variants of our
  approach. Furthermore, we evaluate the proposed approach in a fine-tuning
  scenario and show that it outperforms common fine-tuning methods. Our results
  demonstrate that our proposed approach is a promising alternative to existing
  pruning techniques, providing an efficient and effective way to reduce the
  size of neural networks, while eliminating the need for costly fine-tuning
  step.\\
