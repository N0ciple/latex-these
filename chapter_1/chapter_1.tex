...
\begin{abstract}
  ...
  Conv4, VGG16, ResNet20 on CIFAR10 and CIFAR100, and ResNet18 on the more
...
\begin{algorithm}
  ...
  \FOR{$t = 1$ to $E$}
      \FOR{each $(x,y) \in \mathcal{D}$}
          ...
          \STATE Compute the loss $\mathcal{L}= \mathcal{L}_{task}(y, f(x; \theta_t)) + \lambda \cdot \mathcal{L}^{p}_{budget}(\theta_t)$
