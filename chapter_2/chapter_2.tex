...
\begin{abstract}
  ...
  and TinyImageNet datasets. In most cases, ASLP outperforms other
...
\subsubsection{Extracting Effective Subnetwork}
...
particularly successful at simplifying large neural networks, and seek to remove
...
comparable performances. \citeauthor{DBLP:conf/iclr/LiuSZHD19}
...
Strong \acl{LTH}. A theoretical analysis provided evidence that such subnetworks
do exist. \cite{DBLP:conf/icml/MalachYSS20} demonstrate that a neural network of
width $d$ and depth $l$ can be approximated by pruning a randomly initialised
one that is a factor $O(d^4l^2)$ wider and twice as deep. The  upper bound on
the network width has later been improved by \cite{DBLP:conf/nips/OrseauHR20}
to $O(d^2\log(dl))$ under the assumption of a hyperbolic weight distribution.
This upper bound has eventually been refined to $O(d\log(dl))$ by
...
since this method enforces the pruning rate \textit{a priori}, finding the
pruning rate giving the highest performances has to be made through a cumbersome
and time-consuming binary or grid search. \\
...
flexibility of subnetwork selection and adaptation compared to DWR (which is
...
To accomplish this, we employ the same networks and datasets previously
introduced in \cref{chap:chapter1}, namely, CIFAR-10, CIFAR-100 and TinyImageNet
which are described in \cref{sec:chap1:experiments}. Unless stated otherwise, on
each table we report the test accuracy, given in percentages (plus or minus the
...
Conv2, Conv4, Conv6, VGG16, ResNet-20, and ResNet-18. Conv2 and Conv6 are
modified versions of the Conv4 architecture, featuring 2 and 6 convolutional
layers, respectively, instead of the original 4 convolutional layers. The other
networks mentioned are described in \cref{sec:chap1:experiments}. The number of
parameters for these architectures is provided in
\cref{tab:chap2:conv_num_params}. Although the Conv2, Conv4 and Conv6 networks
...
\noindent\textbf{Signed Constant Distribution (SC).} The signed constant
...
consider two configurations: with (w/) and without (w/o) data augmentation. The
data augmentation we apply has been observed in various state-of-the-art
implementations
...
the original size is extracted from the enlarged image. Lastly, a random
...
Overall, the analysis of our ASLP method results, presented in
...
shows that ASLP outperforms both Edge Popup and Supermask methods on CIFAR-10
...
Nevertheless, a ResNet-18 trained on TinyImagenet with ASLP it outperformed by
...
\cref{tab:chap1:networks_size}). As a result, there are numerous possible weight
...
CIFAR10. It shows that the fraction of explored topologies by our \ac{ASLP}
...
\begin{table}
  ...
  \resizebox{\textwidth}{!}{
    ...
    ResNet-18 networks with CIFAR10 and TinyImageNet, respectively. Since a new
...
\cref{tab:chap1:networks_size}), nevertheles, in our experiments, \ac{ASLP}
achieves superior results compared to other methods on the VGG16 architecture.
We suggest the following explanation: First, the CIFAR10 and CIFAR100 datasets
...
Indeed, a VGG16 network tailored for CIFAR100 has 51,200 parameters in its last
...
\begin{table}[htbp]
  ...
  \begin{subtable}[t]{\textwidth}
    ...
    \resizebox{14cm}{!}{
      \begin{tabular}{llcccc}
        ...
        \toprule
        % Conv 2 results
        \multirow{12}{*}{} \multirow{4}{*}{\textbf{Conv2}} & ASLP (thresholding)                              & \textbf{75.70 $\pm$ 0.30} & \textbf{75.81 $\pm$ 0.69} & \textbf{76.48 $\pm$ 0.68} & \textbf{76.92 $\pm$ 0.24} \\
                                                           & ASLP (averaging)                                 & 75.42 $\pm$ 0.25          & 75.50 $\pm$ 0.56          & 76.05 $\pm$ 0.44          & 76.44 $\pm$ 0.19          \\
        ...
        \midrule
        % Conv 4 results
        \multirow{4}{*}{\textbf{Conv4}}                    & ASLP (thresholding)                              & \textbf{83.03 $\pm$ 0.31} & \textbf{83.73 $\pm$ 0.46} & \textbf{83.59 $\pm$ 0.29} & \textbf{84.06 $\pm$ 0.31} \\
                                                           & ASLP (averaging)                                 & 82.29 $\pm$ 0.25          & 83.22 $\pm$ 0.56          & 82.79 $\pm$ 0.30          & 83.46 $\pm$ 0.49          \\
                                                           & \cite{DBLP:conf/nips/ZhouLLY19} (averaging)      & -                         & -                         & -                         & -                         \\
                                                           & \cite{DBLP:conf/cvpr/RamanujanWKFR20} ($k=50\%$) & 82.38 $\pm$ 0.29          & 83.61 $\pm$ 0.38          & 81.71 $\pm$ 0.59          & 83.55 $\pm$ 0.32          \\
        \midrule
        % Conv 6 results
        \multirow{4}{*}{\textbf{Conv6}}                    & ASLP (thresholding)                              & \textbf{84.98 $\pm$ 0.33} & \textbf{86.49 $\pm$ 0.36} & \textbf{85.32 $\pm$ 0.27} & \textbf{86.21 $\pm$ 0.34} \\
                                                           & ASLP (averaging)                                 & 84.24 $\pm$ 0.28          & 85.67 $\pm$ 0.34          & 84.51 $\pm$ 0.35          & 85.49 $\pm$ 0.38          \\
                                                           & \cite{DBLP:conf/nips/ZhouLLY19} (averaging)      & -                         & -                         & -                         & -                         \\
  ...
  \begin{subtable}[t]{\textwidth}
    ...
    \resizebox{14cm}{!}{
      \begin{tabular}{llcccc}
        ...
        \toprule
        % Conv 2 results
        \multirow{12}{*}{} \multirow{4}{*}{\textbf{Conv2}} & ASLP (thresholding)                              & \textbf{68.24 $\pm$ 0.14} & \textbf{ 68.11 $\pm$ 0.64} & \textbf{66.84 $\pm$ 0.46} & \textbf{66.05 $\pm$ 0.93} \\
                                                           & ASLP (averaging)                                 & 68.09 $\pm$ 0.35          & 67.69 $\pm$ 0.52           & 65.79 $\pm$ 0.65          & 65.35 $\pm$ 0.83          \\
        ...
        \midrule
        % Conv 4 results
        \multirow{4}{*}{\textbf{Conv4}}                    & ASLP (thresholding)                              & \textbf{71.64 $\pm$ 0.36} & \textbf{69.74 $\pm$ 1.37}  & \textbf{72.85 $\pm$ 0.48} & \textbf{72.08 $\pm$ 0.62} \\
                                                           & ASLP (averaging)                                 & 70.88 $\pm$ 0.47          & 68.77 $\pm$ 1.42           & 71.82 $\pm$ 0.53          & 71.09 $\pm$ 0.69          \\
                                                           & \cite{DBLP:conf/nips/ZhouLLY19} (averaging)      & 68.09 $\pm$ 0.84          & 67.48 $\pm$ 0.52           & 58.13 $\pm$ 2.39          & 53.84 $\pm$ 5.00          \\
                                                           & \cite{DBLP:conf/cvpr/RamanujanWKFR20} ($k=50\%$) & -                         & -                          & -                         & -                         \\
        \midrule
        % Conv 6 results
        \multirow{4}{*}{\textbf{Conv6}}                    & ASLP (thresholding)                              & \textbf{73.32 $\pm$ 0.42} & \textbf{69.83 $\pm$ 1.46}  & \textbf{76.20 $\pm$ 0.91} & \textbf{75.30 $\pm$ 0.89} \\
                                                           & ASLP (averaging)                                 & 72.62 $\pm$ 0.57          & 69.53 $\pm$ 1.68           & 75.24 $\pm$ 0.69          & 74.50 $\pm$ 0.96          \\
  ...
  \end{subtable}
  \caption{Comparison of ASLP \textbf{test accuracy} against Edge-Popup and Supermask
    \cite{DBLP:conf/cvpr/RamanujanWKFR20,DBLP:conf/nips/ZhouLLY19} on \textbf{CIFAR10}
...
%     \end{tabular}
%   }
%   \caption{\textbf{OLD TABLE} Comparison of ASLP performance against Edge-Popup and Supermask
...
\begin{table}[htbp]
  ...
  \begin{subtable}[t]{\textwidth}
    ...
    \resizebox{14cm}{!}{
      \begin{tabular}{llllll}
        ...
        \toprule
        \multirow{12}{*}{}
        \multirow{4}{*}{\textbf{Conv2}} & ASLP (thresholding)                              & \textbf{38.64 $\pm$ 0.92} & 38.31 $\pm$ 0.75          & \textbf{41.81 $\pm$ 0.84} & \textbf{42.06 $\pm$ 0.76} \\
        ...
        \multirow{4}{*}{\textbf{Conv4}} & ASLP (thresholding)                              & \textbf{47.78 $\pm$ 1.18} & 49.33 $\pm$ 0.77          & \textbf{50.33 $\pm$ 0.39} & \textbf{51.49 $\pm$ 0.43} \\
        ...
        \multirow{4}{*}{\textbf{Conv6}} & ASLP (thresholding)                              & 51.09 $\pm$ 0.92          & 53.00 $\pm$ 0.52          & \textbf{51.70 $\pm$ 0.48} & 52.85 $\pm$ 0.50          \\
  ...
  \begin{subtable}[t]{\textwidth}
    ...
    \resizebox{14cm}{!}{
      \begin{tabular}{llllll}
        ...
        \toprule
        \multirow{12}{*}{}
        \multirow{4}{*}{\textbf{Conv2}} & ASLP (thresholding)                              & \textbf{38.72 $\pm$ 0.59} & 38.64 $\pm$ 1.23          & \textbf{42.42 $\pm$ 0.30} & \textbf{41.95 $\pm$ 0.68} \\
                                        & ASLP (averaging)                                 & 38.40 $\pm$ 0.81          & \textbf{38.71 $\pm$ 1.05} & 41.66 $\pm$ 0.39          & 41.42 $\pm$ 0.55          \\
                                        & \cite{DBLP:conf/nips/ZhouLLY19} (averaging)      & 38.09 $\pm$ 1.03          & 37.28 $\pm$ 0.47          & 26.03 $\pm$ 2.23          & 23.49 $\pm$ 1.36          \\
                                        & \cite{DBLP:conf/cvpr/RamanujanWKFR20} ($k=50\%$) & -                         & -                         & -                         & -                         \\
        \midrule

        \multirow{4}{*}{\textbf{Conv4}} & ASLP (thresholding)                              & \textbf{47.56 $\pm$ 0.36} & \textbf{49.30 $\pm$ 0.54} & \textbf{50.39 $\pm$ 0.58} & \textbf{51.16 $\pm$ 0.94} \\
        ...
        \multirow{4}{*}{\textbf{Conv6}} & ASLP (thresholding)                              & \textbf{51.43 $\pm$ 0.41} & \textbf{53.10 $\pm$ 0.27} & \textbf{51.52 $\pm$ 0.35} & \textbf{53.22 $\pm$ 0.54} \\
  ...
  \caption{Comparison of ASLP \textbf{test accuracy} against Edge-Popup and Supermask
    \cite{DBLP:conf/cvpr/RamanujanWKFR20,DBLP:conf/nips/ZhouLLY19} on \textbf{CIFAR100}
    using various configurations. We use the configurations tested by the
    authors in their articles. Performances are presented with
    (\cref{tab:chap2:perf-tables:with-augmentation}) and without
    (\cref{tab:chap2:perf-tables:without-augmentation}) data augmentation,
    ...
    Conv2 and Conv4. However, for Conv6, ASLP performance is superior when data
...
\begin{table}[htbp]
  \centering\begin{tabular}{llcc}
    ...
    \bottomrule
  \end{tabular}
  \caption{Comparison of ASLP \textbf{test accuracy} against Edge-Popup and
    ...
    and network architectures, ASLP surpasses the comparative methods in its
...
\begin{table}[htbp]
  \centering\begin{tabular}{lcc}
    ...
    \bottomrule
    \end{tabular}
  \caption{Comparison of the ASLP method observed \textbf{pruning rates} across
...
%     \bottomrule
%   \end{tabular}
%   \caption{\textbf{OLD TABLE}Comparison of the ASLP method \textbf{sparsity levels} across various
