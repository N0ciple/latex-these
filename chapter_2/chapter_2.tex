...
\begin{abstract}
  ...
  referred to as \acf{SR}, is introduced. It rescales the weight distributions
  ...
  complete, eliminating the need to iteratively search and strictly enforce a
  ...
  regarding performance improvement and the reduction in the number of
...
In general, extracting an efficient subnetwork is still a challenging problem
...
$\mathbb{1}$ is the all ones vector. Contrary to previous methods, namely
...
dataset or batch size. Since it prevents layer collapse, network pruning with
...
indicators are not differentiable, therefore, the latter is made with \ac{STE}.
...
approach to obtain the masks $\bm{m}_\ell$ that is not bound to the value of the
weights. In \cref{eqn:chap2:pruned_layer_eq}, the masks $\bm{m}_\ell$ are
stochastic and sampled from a Bernoulli distribution. However, sampling is not a
differentiable operation, therefore, optimising directly ${\bm{m}_\ell}$ is not
possible. To overcome this issue, while still relying on \ac{SGD}, the \acf{STE}
technique is applied together with a reparametrisation of the mask.\\
...
\begin{algorithm}
  ...
  \begin{algorithmic}
    ...
    \COMMENT{Give the masks $\bm{m}_{ij}$ their values}
