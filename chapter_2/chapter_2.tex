...
unpruned network. This pruning fixes the values of the masks $m_{ij}$ to
...
function $\xi_\tau$ to each mask $\hat{m}_{ij}$, and assigning its result
to $m_{ij}$, as shown in \cref{eq:chap2:pruning}.\\
...
automatically determine the optimal pruning rate for an architecture in one shot
