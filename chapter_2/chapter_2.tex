...
Our method is a combination of the \acl{STGS} sampling and the \acl{SR}
mechanism. The two are combined in a global method called \acf{ASLP}. This
method generates lightweight networks by sampling topologies in a large
untrained network and learning the probability of selection of each weight.
Probabilities are learned with \acl{SGD} together with a conventional loss
function (a cross-entropy loss in the case of image classification). \\
...
\begin{algorithm}
  ...
  \label{alg:chap1:training_pruning2}
  \begin{algorithmic}
