...
\begin{abstract}
  ...
  % TODO: écrire l'abstract du chapitre
  ...
  performances and reduces the number of epochs required for training.
  ...
  state-of-the-art methods and consistently surpasses them for various networks
  architectures. Further experiments show that the sparsity of the networks
  extracted with \ac{ASLP} can be increased with a minimal impact on the
  performances, and also demonstrate the robustness of the method to extreme
...
concept and channel shuffling. CondenseNet \cite{huang2018condensenet} leverage
...
since this method enforce the pruning rate \textit{a priori}, finding the pruning
...
fully connected layers since the datasets it is evaluated on has fewer classes.
...
independent runs and the shaded area of the corresponding color indicates the
...
% Future research could explore the application of \acl{ASLP} to other neural
% network architectures and tasks or investigate additional techniques for
% further improving subnetwork selection and adaptation.
