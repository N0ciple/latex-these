\chapter{\DIFdelbegin \DIFdel{Mask }\DIFdelend \DIFaddbegin \DIFadd{Effective Subnetworks Extraction without Weight }\DIFaddend Training}
\label{chap:chapter2}
...
standard training. \DIFaddbegin \DIFadd{The following sections provide an overview of the related
works on pruning at initialisation, Lottery Tickets, effective subnetwork
existence and extraction, followed by the contributions of this chapter.}\DIFaddend \\


%DIF < TODO: compléter l'intérêt de ce nouveau "chemin" ?
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend % \subsection{Related Work}\label{sec:chap2:related_work}
...
from the Bernoulli distribution parametrised by $m$, \DIFdelbegin \DIFdel{that }\DIFdelend \DIFaddbegin \DIFadd{which }\DIFaddend either selects or
...
following the \ac{SGD} algorithm, vanishingly small gradients \DIFdelbegin \DIFdel{results }\DIFdelend \DIFaddbegin \DIFadd{result }\DIFaddend in
...
\cite{glorot2011deep} that the author used. \DIFdelbegin \DIFdel{Experimentaly}\DIFdelend \DIFaddbegin \DIFadd{Experimentally}\DIFaddend , a larger standard
...
presents a pruning strategy to evaluate \DIFaddbegin \DIFadd{a }\DIFaddend network trained with our method on a
...
network is required due to limited computing \DIFdelbegin \DIFdel{ressources }\DIFdelend \DIFaddbegin \DIFadd{resources }\DIFaddend or where weight training
may not be feasible.\\
...
\begin{figure}[h!]
  ...
  \caption{Data Augmentation \DIFdelbeginFL \DIFdelFL{pipeleine }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{pipeline }\DIFaddendFL example used for CIFAR-10 and CIFAR-100.}
...
retention of \DIFaddbegin \DIFadd{the }\DIFaddend most valuable connections in the network. This approach focuses
on the utilisation of \DIFdelbegin \DIFdel{high impact }\DIFdelend \DIFaddbegin \DIFadd{high-impact }\DIFaddend weights, which \DIFdelbegin \DIFdel{contributes }\DIFdelend \DIFaddbegin \DIFadd{contribute }\DIFaddend to the enhanced
...
\DIFdelbegin \DIFdel{network overall performance }\DIFdelend \DIFaddbegin \DIFadd{overall performance of the network}\DIFaddend . The inclusion of such potentially useless
...
method \DIFdelbegin \DIFdel{, }\DIFdelend is considerably higher for the Conv4 network than for the ResNet-18 one
...
%DIFDELCMD <       \cmidrule[\heavyrulewidth]{2-3}
%DIFDELCMD <                                                          & \textbf{Conv4 \&
%DIFDELCMD <         CIFAR-10}
%DIFDELCMD <                                                          &
%DIFDELCMD <       \textbf{ ResNet-18 \& TinyImageNet}
%DIFDELCMD <       \\
%DIFDELCMD <       \midrule
%DIFDELCMD <       Number of parameters ($N$)                         & 2,425,930                            & 11,685,608 \\
%DIFDELCMD <       Train Dataset Size (number or images)              & 50,000                               & 90,000     \\
%DIFDELCMD <       Batch Size used for training                       & 256                                  & 256        \\
%DIFDELCMD <       Nb. of batches for 1 epoch                         & 196                                  & 352        \\
%DIFDELCMD <       Nb. of explored topologies in $10^3$ epochs ($E$)  & 196,000                              & 352,000    \\
%DIFDELCMD <       Nb. of possible topologies ($P$)                   & $P=2^N \approx
%DIFDELCMD <       5\times 10^{10^{5.86}}$                    & $P=2^N \approx 5\times
%DIFDELCMD <       10^{10^{6.54}}$              \\
%DIFDELCMD <       Fraction of explored topologies ($\nu_\text{exp}$) &
%DIFDELCMD <       $\nu_\text{exp}^\text{Conv4} = \displaystyle\frac{E}{P} \approx
%DIFDELCMD <       10^{{-10}^{5.86}}$                                 & $\nu_\text{exp}^\text{ResNet-18} =
%DIFDELCMD <       \displaystyle\frac{E}{P} \approx 10^{{-10}^{6.54}}$                                                    \\
%DIFDELCMD <       Ratio of fraction of explored topologies           &
%DIFDELCMD <       \multicolumn{2}{c}{$\displaystyle\frac{\nu_\text{exp}^\text{Conv4}}{\nu_\text{exp}^\text{ResNet-18}}
%DIFDELCMD <       \approx 10^{10^{6.44}}$}                                                                               \\
...
\DIFdelendFL \DIFaddbeginFL \resizebox{\textwidth}{!}{
    \begin{tabular}{lcc}
      ...
      \bottomrule
    \end{tabular}
  } \DIFaddendFL \caption {Comparison of the number of explored topologies for the Conv4 and
...
\cref{tab:dlo:networks_size}), \DIFdelbegin \DIFdel{nevertheles}\DIFdelend \DIFaddbegin \DIFadd{nevertheless}\DIFaddend , in our experiments, \ac{ASLP}
...
are simpler datasets with \DIFdelbegin \DIFdel{less }\DIFdelend \DIFaddbegin \DIFadd{fewer }\DIFaddend classes, compared to TinyImageNet. Secondly,
...
In standard training, high learning rate values \DIFdelbegin \DIFdel{causes }\DIFdelend \DIFaddbegin \DIFadd{cause }\DIFaddend the optimisation to fail
because of parameters becoming too large and resulting in \ac{nan} values.
However, this is not the case for \ac{ASLP} which \DIFdelbegin \DIFdel{exhibit }\DIFdelend \DIFaddbegin \DIFadd{exhibits }\DIFaddend robustness to high
...
automatically \DIFdelbegin \DIFdel{determine }\DIFdelend \DIFaddbegin \DIFadd{determines }\DIFaddend the optimal pruning rate for an architecture in one shot
