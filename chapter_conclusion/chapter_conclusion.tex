...
training \DIFaddbegin \DIFadd{allows for joint optimisation of the weights and the topology.
Moreover, by controlling the number of parameters that will remain, it
}\DIFaddend encourages the network not to use more capacity and therefore weights than what
...
importance of budget enforcement \DIFaddbegin \DIFadd{and weight reparametrisation}\DIFaddend , even on already
...
approaches, by design, cannot treat differently two \DIFdelbegin \DIFdel{weight that has the same
}\DIFdelend \DIFaddbegin \DIFadd{connections with the same
weight }\DIFaddend value. In \cref{chap:chapter2}, we proposed a new stochastic approach to
extract lightweight subnetworks from a large untrained network. This approach
estimates the importance of a weight based on trained masks \DIFdelbegin \DIFdel{that }\DIFdelend \DIFaddbegin \DIFadd{which }\DIFaddend are auxiliary
