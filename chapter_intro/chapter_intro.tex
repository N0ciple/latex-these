...
%DIF <  - révolutions industrielles de la première à la 4ème avec pour objectifs la mecanisation du travail dans la première (détailler les 3 autres)
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  - composante de la 4ème révolution industrielle : 
%DIF <      - l'intelligence artificielle permise par le développement des capacités de calcul
%DIF <      - naissance d'objects intelligents et doté de capacités de calcul embarqués (smartphones, voitures autonomes, satellites, etc...)
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  - la taille des modèles est un frein au déploiement de l'intelligence artificielle sur des objets embarqués.
%DIF <  - CEPENDANT: la 4ème révolution industrielle est aussi celle des objets communiquants donc on pourrait déporter les calculs sur des serveurs distants.
%DIF <  - MAIS: il existe des raisons de vouloir faire du calcul embarqué:
%DIF <      - confidentialité des données
%DIF <      - distribuer la charge de calcul et ne pas assumer le coût, ni de la communication, ni du calcul
%DIF <      - plus grand réactivité (temps de latence)
%DIF <      - coût de la communication
%DIF <      - pas necessairement d'accès à internet (autonomie: ex: rover sur Mars)
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend From the spinning jenny, blast furnace and steam engine that sparked the first
...
\DIFdelbegin \DIFdel{processing}\DIFdelend \DIFaddbegin \DIFadd{representation}\DIFaddend . The fast development of Deep learning has been driving advancements
...
\DIFdelbegin \DIFdel{computation }\DIFdelend \DIFaddbegin \DIFadd{computations }\DIFaddend instead of moving them to the cloud. First, processing the data
locally on premises ensures better data privacy, since the latter does not need
to leave the device \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{to }\DIFaddend be processed on the cloud. Indeed cloud instances can be
located on various continents \DIFaddbegin \DIFadd{or countries }\DIFaddend where the legislation about data
...
second \DIFdelbegin \DIFdel{method }\DIFdelend \DIFaddbegin \DIFadd{approach }\DIFaddend does not require any weight training and instead focuses on
stochastic yet differentiable topology selection, achieving compelling results
overall and outperforming other \DIFaddbegin \DIFadd{related }\DIFaddend state-of-the-art methods that, again, do
...
previous section, particularly in ensuring data privacy\DIFdelbegin \DIFdel{and allowing }\DIFdelend \DIFaddbegin \DIFadd{. Moreover, it allows }\DIFaddend for
...
}\DIFdelend \cite{DBLP:conf/cvpr/HeZRS16,DBLP:conf/nips/RenHGS15,DBLP:conf/eccv/LiuAESRFB16}.
...
learn gradually more complex and abstract concept representations from \DIFdelbegin \DIFdel{the }\DIFdelend raw data,
...
\DIFdelbegin \DIFdel{set of
}\DIFdelend challenges that need to be addressed, especially in the context of deploying
these \DIFaddbegin \DIFadd{deep and large }\DIFaddend models on embedded devices. These challenges include model
complexity and computational requirements. The necessity of compressing \DIFdelbegin \DIFdel{the neural
networks have, highlighted previously , }\DIFdelend \DIFaddbegin \DIFadd{neural
networks has been highlighted previously and }\DIFaddend also comes with its \DIFdelbegin \DIFdel{own set of challenges that
comprises the following}\DIFdelend \DIFaddbegin \DIFadd{challenges that
include}\DIFaddend : preserving the performance \DIFdelbegin \DIFdel{,  }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend controlling the size of the compressed
model as well as \DIFdelbegin \DIFdel{the }\DIFdelend training time.\\

One of the most significant challenges in deploying deep learning models \DIFdelbegin \DIFdel{,
}\DIFdelend \DIFaddbegin \DIFadd{and
}\DIFaddend especially \acp{CNN} \DIFdelbegin \DIFdel{, }\DIFdelend on embedded devices \DIFdelbegin \DIFdel{, }\DIFdelend is the large model size. These models
often have millions of parameters \DIFdelbegin \DIFdel{, making }\DIFdelend \DIFaddbegin \DIFadd{and this makes }\DIFaddend them computationally heavy and
challenging to fit into the limited memory of embedded devices. \DIFdelbegin %DIFDELCMD < \\
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend Secondly, these
complex models require substantial computational resources to operate. This
translates \DIFdelbegin \DIFdel{to slow computations and processing times, }\DIFdelend \DIFaddbegin \DIFadd{into slow computations which is }\DIFaddend a critical issue for devices which
aim to perform real-time tasks.\\

Compressing \DIFdelbegin \DIFdel{the large neural netwoks }\DIFdelend \DIFaddbegin \DIFadd{large neural networks }\DIFaddend is a necessity to deploy them on embedded
devices. However, this compression process comes with its \DIFdelbegin \DIFdel{own set of }\DIFdelend challenges. First, the
compressed model should maintain the performance of the original model. However\DIFaddbegin \DIFadd{,
}\DIFaddend the original large model is trained \DIFdelbegin \DIFdel{to rely on }\DIFdelend \DIFaddbegin \DIFadd{with }\DIFaddend all its parameters and \DIFaddbegin \DIFadd{thus depends on
all of them. Consequently, }\DIFaddend removing more than a \DIFdelbegin \DIFdel{trivial number of parameters }\DIFdelend \DIFaddbegin \DIFadd{few }\DIFaddend can lead to degraded
...
To conclude, while deep learning and \DIFdelbegin \DIFdel{CNNs }\DIFdelend \DIFaddbegin \acp{CNN} \DIFaddend represent an exciting \DIFdelbegin \DIFdel{frontier }\DIFdelend \DIFaddbegin \DIFadd{advancement
}\DIFaddend in computer vision applications, several challenges need to be addressed for
...
determining the saliency of the weights\DIFdelbegin \DIFdel{. The latter }\DIFdelend \DIFaddbegin \DIFadd{, which }\DIFaddend is a score that reflects the
importance of the \DIFdelbegin \DIFdel{weight }\DIFdelend \DIFaddbegin \DIFadd{weights }\DIFaddend in the network. The second challenge is often
sidestepped and the pruned network is simply fine-tuned to recover the lost
performance. To address \DIFdelbegin \DIFdel{these challenges }\DIFdelend \DIFaddbegin \DIFadd{both of these challenges, }\DIFaddend we propose the following main
contributions:

\begin{itemize}
      \item A numerically stable reparametrisation function, used in \DIFaddbegin \DIFadd{both }\DIFaddend our
            weight reparametrisation and \DIFaddbegin \DIFadd{our }\DIFaddend budget regularisation loss
      ...
      \item A weight reparametrisation that embeds the saliency score of the
            ...
            this \DIFdelbegin \DIFdel{reparametrization }\DIFdelend \DIFaddbegin \DIFadd{reparametrisation }\DIFaddend does not require the introduction of auxiliary
      ...
      \item A budget regularisation loss that allows to drive the optimisation
            procedure to respect a given budget. This budget regularisation \DIFaddbegin \DIFadd{loss
            }\DIFaddend benefits directly from the aforementioned reparametrisation function
            to compute the current weight budget. \DIFdelbegin \DIFdel{The budget regularisation loss
            }\DIFdelend \DIFaddbegin \DIFadd{It }\DIFaddend is optimised jointly with
...
the weights, the latter being fixed throughout the process. Instead, we sample \DIFaddbegin \DIFadd{a
}\DIFaddend subset of weights (effectively pruning the other weights) forming a subnetwork
...
network without training \DIFdelbegin \DIFdel{the network}\DIFdelend \DIFaddbegin \DIFadd{its weights}\DIFaddend . The main contributions of this method are
as follows: \\
...
\begin{itemize}
      ...
      \item A pruning strategy for the masks that freeze the topology and
            \DIFdelbegin \DIFdel{perfroms better than previous averaging methods }\DIFdelend \DIFaddbegin \DIFadd{performs better than averaging methods previously }\DIFaddend used in the
            ...
            costly grid search to determine \DIFdelbegin \DIFdel{the optimal pruning rate}\DIFdelend \DIFaddbegin \DIFadd{it}\DIFaddend .
      ...
      \item An efficient learnt-based weight rescaling mechanism to compensate
            ...
            stochastic sampling. This \DIFdelbegin \DIFdel{learnt-based }\DIFdelend rescaling is less computationally
      ...
      \item A comprehensive set of experiments that demonstrate the
            ...
            other \DIFaddbegin \DIFadd{closely related }\DIFaddend state-of-the-art methods in various
            configurations.
...
overview of its foundational and core concepts. It \DIFaddbegin \DIFadd{first }\DIFaddend explores early
...
paradigm, highlighting methods that rely on a \DIFdelbegin \DIFdel{pre-trained large }\DIFdelend \DIFaddbegin \DIFadd{large pre-trained }\DIFaddend network to
...
as experiments validating the components of our \DIFdelbegin \DIFdel{methods }\DIFdelend \DIFaddbegin \DIFadd{method }\DIFaddend and the choices of
...
various scenarios \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend validates the components of our method. The chapter
