...
\begin{figure}[htbp]
  ...
  \caption{Conceptual scheme of the Probabilistic Knowledge Transfer method.
    Both the student and the teacher feature maps are \DIFdelbeginFL \DIFdelFL{modeled }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{modelled }\DIFaddendFL using probability
...
discover effective and efficient architectures \DIFdelbegin \DIFdel{, }\DIFdelend and is thus a critical aspect of
...
efficiency by reducing \DIFaddbegin \DIFadd{the }\DIFaddend computational resources needed with a minimal or
...
quantisation enables \DIFdelbegin \DIFdel{to quantise
}\DIFdelend \DIFaddbegin \DIFadd{quantising }\DIFaddend weights with a larger dynamic range compared to
...
%DIF <  FIXME: ajouter des méthodes de quantisation non uniforme qui ne sont pas basé
%DIF <  sur les puissances de 2.
...
\cref{fig:sota:binarised_kernels}). DoReFa-Net \cite{zhou2016dorefa} \DIFdelbegin \DIFdel{build }\DIFdelend \DIFaddbegin \DIFadd{built }\DIFaddend upon
...
\noindent\textbf{Effective subnetworks.} More recently, \DIFdelbegin \DIFdel{the unstructured pruning
research focuses }\DIFdelend \DIFaddbegin \DIFadd{unstructured pruning
researches focus }\DIFaddend on the discovery of small sub-networks inside the original
...
\cite{DBLP:conf/icml/MalachYSS20} and the requirements \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{on }\DIFaddend the theoretical size
of the original network \DIFdelbegin \DIFdel{has }\DIFdelend \DIFaddbegin \DIFadd{have }\DIFaddend been later refined in
...
\begin{itemize}
  ...
  %DIF <  FIXME: déplacer ce point dans l'introduction. c'est plus une justification
  %DIF <  pour avoir des petits réseaux que pour l'utilisation du pruning.
  \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD <   %%%
%DIF <  \item Then, pruning yields compact and lightweight neural networks. These
  %DIF <  networks are easier to transmit and store in embedded devices. This advantage
  %DIF <  is significant since companies are typically charged for bandwidth, therefore
  %DIF <  deploying a large neural network to hundreds of thousands of devices can come
  %DIF <  at a significant cost. Moreover, the storage capabilities of the said embedded
  %DIF <  devices can be limited and compact networks ease the pressure on storage
  %DIF <  resources.
%DIFDELCMD < 

%DIFDELCMD <   %%%
\DIFdelend \item Then, pruning integrates well with other compression techniques and can
  be applied in conjunction with them, on any kind of \DIFdelbegin \DIFdel{architectures}\DIFdelend \DIFaddbegin \DIFadd{architecture}\DIFaddend .
...
auxiliary variables. As a result, the obtained lightweight networks \DIFdelbegin \DIFdel{is robust to
}\DIFdelend \DIFaddbegin \DIFadd{preserve
accuracy after }\DIFaddend effective pruning and \DIFdelbegin \DIFdel{does }\DIFdelend \DIFaddbegin \DIFadd{do }\DIFaddend not require fine-tuning. Furthermore,
...
also sidestepping the fine-tuning. \DIFdelbegin \DIFdel{Contrary }\DIFdelend \DIFaddbegin \DIFadd{In contrast }\DIFaddend to other methods, the optimal
