...
% TODO: changer les sections quand on aura déplacé les parties dans l'intro
\cref{sec:sota:dl_overview} provides a brief overview of deep learning and
neural network evolution. \Cref{sec:sota:nn_evolution} underscores the increase
in size of the most popular architectures. Following this line,
...
\section{Neural Networks Size and Achitecture Evolution}\label{sec:sota:nn_evolution}
...
%TODO: retravailler le titre de la section
...
% TODO: ajouter des méthodes de quantisation non uniforme qui ne sont pas basé
...
\noindent\textbf{} More recently, the unstructured pruning research focuses on the discovery of
...
weights. These smaller networks, found by pruning the smallest magnitude weights
from the trained original network, are observed to train faster and achieve
comparable or even superior performance with respect to the original network.
The \emph{lottery ticket} is found by training the original network up to
...
works: \cite{DBLP:conf/nips/ZhouLLY19} proposed an analysis of the results
presented in \cite{DBLP:conf/iclr/FrankleC19}, These results
\cite{DBLP:conf/iclr/FrankleC19} have been extended to larger networks in
\cite{DBLP:journals/corr/abs-1903-01611}, the necessity of training the original
network to convergence to find the Lottery Tickets has been challenged in
\cite{DBLP:conf/iclr/LiuSZHD19}. The existence of the \emph{lottery ticket} (or
in other words the subnetwork) has been theoretically proven in
...
First, we explored the historical progression and the major architectures of
...
network to a smaller, more efficient one. The core idea is to teach a
lightweight student network to mimic the behaviour of a teacher network, thus
achieving comparable performance with a reduced footprint. Next, we delved into
efficient architecture design methods, including bespoke architectures designed
to minimise size while maintaining performance, and \acl{NAS} strategies for
automating the discovery of optimal architectures. Lastly, we addressed the
strategies for compressing and optimising existing neural networks, considering
both quantisation and binarisation techniques that lower the numerical precision
of weights and activations, as well as pruning techniques that remove redundant
or insignificant weights and connections, resulting in a sparser, more
...
% \begin{tikzpicture}[
%     mindmap,
%     every node/.style={concept, execute at begin node=\hskip0pt, fill=white, drop shadow},
%     root concept/.append style={
%         concept color=black, fill=white, line width=0.5ex, text=black, font=\large\scshape
%       },
%     text=black,
%     grow cyclic,
%     level 1 concept/.append style={level distance=4.5cm,sibling angle=90,font=\normalsize},
%     level 2 concept/.append style={level distance=3cm,sibling angle=45,font=\small},
%     level 3 concept/.append style={level distance=2.5cm,sibling angle=30,font=\footnotesize} % Added level 3 with decreased level distance and font size
%   ]
%   \node [root concept] {Deep Neural Network Compression and Acceleration}
%   child [concept color=blue!30] {
%       node {Accelerating Computations}
%       child { node {Algorithm}
%           child { node {Method 1}} % New level 3 child
%           child { node {Method 2}} % New level 3 child
%         }
%       child { node {Data Structures \cite{4531741}}}
%     }
%   child [concept color=red!30] {
%       node {Teaching paradigm}
%       child { node {Linux}}
%       child { node {Windows}}
%     }
%   child [concept color=orange!30] {
%       node {Architecture Design}
%       child { node {Requirements Analysis}}
%       child { node {System Design}}
%     }
%   child [concept color=green!30!black] {
%       node {Compressing and Optimising an Existing Architecture}
%       child { node {Quantisation}}
%       child { node {NoSQL}}
%     };
