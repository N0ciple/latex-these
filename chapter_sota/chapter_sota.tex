...
are highly constrained in terms of computing power, energy consumption \DIFdelbegin \DIFdel{or }\DIFdelend \DIFaddbegin \DIFadd{and
}\DIFaddend memory footprint. Alongside the increase in the size of these networks,
...
particularly with large and complex networks. This may lead to \DIFdelbegin \DIFdel{significant
processing }\DIFdelend \DIFaddbegin \DIFadd{long computation
}\DIFaddend times, posing a challenge for real-time or resource-limited applications. To
...
%DIFDELCMD <     P_{1} & = (A_{11} + A_{22})(B_{11} + B_{22}) \\
%DIFDELCMD <     P_{2} & = (A_{21} + A_{22})B_{11}            \\
%DIFDELCMD <     P_{3} & = A_{11}(B_{12} - B_{22})            \\
%DIFDELCMD <     P_{4} & = A_{22}(B_{21} - B_{11})            \\
%DIFDELCMD <     P_{5} & = (A_{11} + A_{12})B_{22}            \\
%DIFDELCMD <     P_{7} & = (A_{21} - A_{11})(B_{11} + B_{12}) \\
%DIFDELCMD <     P_{6} & = (A_{12} - A_{22})(B_{21} + B_{22})
...
\cite{DBLP:journals/corr/IandolaMAHDK16}. These modules\DIFaddbegin \DIFadd{, represented in
}\cref{fig:sota:fire_module}\DIFadd{, }\DIFaddend aim to minimise computational requirements by
...
to the \emph{Expand Layer}, subsequently reducing the \DIFdelbegin \DIFdel{parameter count }\DIFdelend \DIFaddbegin \DIFadd{number of parameters }\DIFaddend in
...
$1\times1$ variants. Although the \DIFdelbegin \DIFdel{$1x1$ }\DIFdelend \DIFaddbegin \DIFadd{$1\times1$ }\DIFaddend convolutions capture less spatial
information, they are significantly less computationally demanding than the
\DIFdelbegin \DIFdel{$3x3$ }\DIFdelend \DIFaddbegin \DIFadd{$3\times3$ }\DIFaddend ones.\\
...
split. Strided convolution helps \DIFdelbegin \DIFdel{reducing the spatial dimensions }\DIFdelend \DIFaddbegin \DIFadd{to reduce the spatial extent }\DIFaddend of feature maps,
...
\begin{figure}[htbp]
  ...
  lower \DIFdelbeginFL \DIFdelFL{channel count }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{number of channels }\DIFaddendFL are the ones connected via the skip connection,
...
and performance of \DIFdelbegin \DIFdel{networks architecture }\DIFdelend \DIFaddbegin \DIFadd{network architectures }\DIFaddend detailed in this section can be
compared to standard architecture sizes \DIFdelbegin \DIFdel{on }\DIFdelend \DIFaddbegin \DIFadd{in }\DIFaddend \cref{fig:sota:net_sizes_std_eff}.\\
...
it is possible to \DIFdelbegin \DIFdel{contrain }\DIFdelend \DIFaddbegin \DIFadd{constrain }\DIFaddend neural networks to use lower precision values,
...
% FIXME: ajouter des méthodes de quantisation non uniforme qui ne sont pas basé
...
\DIFdelbegin \DIFdel{$n+1$ }\DIFdelend \DIFaddbegin \DIFadd{$\ell+1$ }\DIFaddend implies pruning a filter in layer \DIFdelbegin \DIFdel{$n$}\DIFdelend \DIFaddbegin \DIFadd{$\ell$}\DIFaddend , and vice-versa). This type of
...
an analysis of the results presented in \cite{DBLP:conf/iclr/FrankleC19}\DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{. }\DIFaddend These
...
\begin{itemize}
  ...
  % FIXME: déplacer ce point dans l'introduction. c'est plus une justification
  % pour avoir des petits réseaux que pour l'utilisation du pruning.
  ...
  be applied in conjunction \DIFaddbegin \DIFadd{with }\DIFaddend them, on any kind of architectures.
...
pruning method that circumvents the need for the expensive fine-tuning step. \DIFaddbegin \DIFadd{Its
...
auxiliary variables. As a result, the obtained lightweight networks is robust to
effective pruning and does not require fine-tuning. }\DIFaddend Furthermore,
...
also sidestepping the fine-tuning. \DIFaddbegin \DIFadd{Contrary to other methods, the optimal
pruning rate is discovered in one shot by our pruning strategy that circumvents
the costly gird search for its value. }\DIFaddend This innovative approach opens up new
possibilities for further reducing the computational costs associated with
pruning and provides a new direction for future research in this area\DIFaddbegin \DIFadd{, as well
as neural network training without weight tuning in general}\DIFaddend .\\
...
budget. This method allows \DIFdelbegin \DIFdel{to bypass }\DIFdelend \DIFaddbegin \DIFadd{bypassing }\DIFaddend the need for the finetuning step present in
most methods based on magnitude pruning \DIFaddbegin \DIFadd{by jointly optimising the topology and
the weights without the need for additional auxiliary parameters}\DIFaddend . 